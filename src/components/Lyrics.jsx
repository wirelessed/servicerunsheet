...
class Lyrics extends Component {
    ...
    componentWillMount() {
        ...
        var ref = firebase.database().ref("services/"+this.props.serviceKey+"/lyrics");
    ...
    componentDidMount() {
        if(this.state.items.length == 0){
            var newItem = firebase.database().ref("services/"+this.props.serviceKey+"/lyrics").push();
    ...
    removeItem = (key) => {
        var firebaseRef = firebase.database().ref("services/"+this.props.serviceKey+'/lyrics');
        ...
        return (
            <div>
                { this.state.items.map((item, index) => {
                        ...
                        if (this.state.editMode){
                            textfield = <TextField name="Text" hintText="Enter Lyrics (free text field)" onChange={this.onExistingTextChange.bind(this, key)} value={ item.text } multiLine={true} rowsMax={999} underlineShow={false} textareaStyle={TextFieldEditStyle} style={TextFieldEditWrapper} />
                        }
                ...
                { (this.state.editMode && this.state.items.length == 0) ? // add new form at the bottom
                    ...
                    <form onSubmit={ this.handleSubmit } style={{ backgroundColor: grey200, padding: '16px 0px'}}>
                        <TextField name="Text" onChange={ this.onTextChange } value={ this.state.text } hintText="Enter Lyrics (free text field)" multiLine={true} rowsMax={999} textareaStyle={TextFieldEditStyle} style={TextFieldEditWrapper} />
...
reactMixin(Lyrics.prototype, ReactFireMixin);
